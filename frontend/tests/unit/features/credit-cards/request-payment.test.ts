...
describe("requestPayment", () => {
  ...
  it("posts the cents amount and returns the accepted request", async () => {
    ...
    const accepted = await requestPayment("ca-1", { amount: 20000 });

    expect(accepted).toEqual({ request_id: "r1", status: "pending" });
    const [, init] = fetchSpy.mock.calls[0] as [string, RequestInit];
    expect(JSON.parse(init.body as string)).toEqual({ amount: 20000 });
  });
  ...
  it("throws an ApiError carrying the response status on a 409 (no active card)", async () => {
    ...
    await expect(requestPayment("ca-1", { amount: 100 })).rejects.toMatchObject({ status: 409 });
    await expect(requestPayment("ca-1", { amount: 100 })).rejects.toBeInstanceOf(ApiError);
  });
});
