"""Postgres implementation of the two `cuentas` ports.

`PostgresCuentaRepository` implements `ICuentaRepository` and has no code path
that writes `saldo`. `PostgresCuentaBalanceProjection` implements
`ICuentaBalanceProjection` and is the only thing in the system that does; it is
constructed once, in `main.py`, and handed only to the balance consumer.
"""
from __future__ import annotations

import datetime as dt
import logging
import secrets
from typing import Optional
from uuid import UUID

from sqlalchemy import select, update as sql_update
from sqlalchemy.exc import IntegrityError

from ....domain.exceptions import DuplicateAccountNumberError
from ....domain.model import NUMERO_CUENTA_LENGTH, Cuenta, EstadoCuenta
from ..errors import translate
from ..interfaces.common import Page
from ..models import CuentaORM
from ._base import PostgresRepository, page_of

LOG = logging.getLogger("openbankapi.cuentas")

# How many times to re-roll a colliding account number before giving up. With
# 10^16 possibilities a single collision is already improbable; three in a row
# means something is wrong with the generator, not with luck.
_GENERATION_ATTEMPTS = 5


def generate_numero_cuenta() -> str:
    """16 random digits, leading zeros preserved.

    `secrets` rather than `random`: this value is a publicly quoted account
    identifier, and a predictable sequence would let anyone enumerate accounts.
    """
    return "".join(secrets.choice("0123456789") for _ in range(NUMERO_CUENTA_LENGTH))


def _to_domain(row: CuentaORM) -> Cuenta:
    return Cuenta(
        id=row.id,
        numero_cuenta=row.numero_cuenta,
        moneda=row.moneda,
        cliente_id=row.cliente_id,
        sucursal_id=row.sucursal_id,
        saldo=row.saldo,
        estado=EstadoCuenta(row.estado),
        created_at=row.created_at,
        updated_at=row.updated_at,
    )


class PostgresCuentaRepository(PostgresRepository):
    # Belt and braces alongside the DTO: even an internal caller cannot name
    # saldo here, because _update is only ever fed these keys.
    _UPDATABLE = frozenset({"moneda", "sucursal_id", "estado"})

    async def create(self, *, moneda: str, cliente_id: UUID, sucursal_id: UUID) -> Cuenta:
        """Insert with a server-generated number, retrying on collision.

        The retry is what keeps spec §11.1 true: a UNIQUE violation on the
        generated number is an internal detail and must never reach the client
        as a 500. A violation on cliente_id/sucursal_id is a different thing
        entirely — that is the caller's bad input, so it is translated and
        raised immediately rather than retried.
        """
        last: Optional[DuplicateAccountNumberError] = None
        for attempt in range(_GENERATION_ATTEMPTS):
            numero = generate_numero_cuenta()
            values = {
                "numero_cuenta": numero,
                "moneda": moneda,
                "cliente_id": cliente_id,
                "sucursal_id": sucursal_id,
            }
            try:
                async with self._sessionmaker.begin() as session:
                    row = CuentaORM(**values)
                    session.add(row)
                    await session.flush()
                    await session.refresh(row)
                    return _to_domain(row)
            except IntegrityError as error:
                translated = translate(error, values=values)
                if not isinstance(translated, DuplicateAccountNumberError):
                    raise translated from error
                last = translated
                LOG.warning("numero_cuenta collision on attempt %d; retrying", attempt + 1)
        raise last if last else RuntimeError("account creation failed without a cause")

    async def get_by_numero(self, numero_cuenta: str) -> Optional[Cuenta]:
        row = await self._fetch_one(CuentaORM, CuentaORM.numero_cuenta == numero_cuenta)
        return _to_domain(row) if row else None

    async def list(self, *, limit: int, offset: int) -> Page:
        rows, total = await self._fetch_page(CuentaORM, limit=limit, offset=offset)
        return page_of([_to_domain(r) for r in rows], total, limit, offset)

    async def update(
        self,
        numero_cuenta: str,
        *,
        moneda: Optional[str] = None,
        sucursal_id: Optional[UUID] = None,
        estado: Optional[str] = None,
    ) -> Optional[Cuenta]:
        candidate = {"moneda": moneda, "sucursal_id": sucursal_id, "estado": estado}
        assert set(candidate) <= self._UPDATABLE, "saldo is not updatable here"
        row = await self._update(
            CuentaORM, CuentaORM.numero_cuenta == numero_cuenta, candidate
        )
        return _to_domain(row) if row else None

    async def close(self, numero_cuenta: str) -> Optional[Cuenta]:
        row = await self._update(
            CuentaORM,
            CuentaORM.numero_cuenta == numero_cuenta,
            {"estado": EstadoCuenta.CERRADA.value},
        )
        return _to_domain(row) if row else None


class PostgresCuentaBalanceProjection(PostgresRepository):
    """The only writer of `cuentas.saldo` (spec §3.6).

    Kept as a separate class so the capability travels separately: `main.py`
    hands one of these to the `account-balances` consumer and to nothing else.
    """

    async def apply_balance(self, numero_cuenta: str, balance: int) -> bool:
        async with self._sessionmaker.begin() as session:
            result = await session.execute(
                sql_update(CuentaORM)
                .where(CuentaORM.numero_cuenta == numero_cuenta)
                .values(saldo=balance, updated_at=dt.datetime.now(dt.timezone.utc))
            )
            # rowcount 0 means the ledger knows an account that reference data
            # does not. That is legitimate, not an error — see the port docs.
            return bool(result.rowcount)

    async def read_saldo(self, numero_cuenta: str) -> Optional[int]:
        """Only used by tests and diagnostics."""
        async with self._sessionmaker() as session:
            return await session.scalar(
                select(CuentaORM.saldo).where(CuentaORM.numero_cuenta == numero_cuenta)
            )
