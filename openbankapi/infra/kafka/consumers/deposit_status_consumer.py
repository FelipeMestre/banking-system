"""The single long-lived consumer on `deposit-status` (admin-cash-deposits).
...
class DepositStatusConsumer:
    ...
    def start(self, _loop: asyncio.AbstractEventLoop) -> None:
        self._thread = threading.Thread(target=self._run, name="deposit-status", daemon=True)
        self._thread.start()
    ...
    def _run(self) -> None:
        ...
        LOG.info("consuming %s", self._settings.deposit_status_topic)
        try:
