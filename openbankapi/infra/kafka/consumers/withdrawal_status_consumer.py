"""The single long-lived consumer on `withdrawal-status` (admin-cash-withdrawals).
...
class WithdrawalStatusConsumer:
    ...
    def start(self, _loop: asyncio.AbstractEventLoop) -> None:
        self._thread = threading.Thread(target=self._run, name="withdrawal-status", daemon=True)
        self._thread.start()
    ...
    def _run(self) -> None:
        ...
        consumer.subscribe([self._settings.withdrawal_status_topic])
        LOG.info("consuming %s", self._settings.withdrawal_status_topic)
        try:
