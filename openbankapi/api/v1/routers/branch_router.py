...
async def create(
    body: BranchCreateDTO, repository: BranchRepositoryDep, _claims: WriteAdminDep
):
...
async def list_all(
    _claims: ReadAdminDep, repository: BranchRepositoryDep, page: PageParams = Depends()
):
...
async def get(
    branch_id: UUID,
    repository: BranchRepositoryDep,
    cache: CacheDep,
...
async def update(
    branch_id: UUID, body: BranchUpdateDTO, repository: BranchRepositoryDep, cache: CacheDep, _claims: WriteAdminDep
):
...
async def soft_delete(branch_id: UUID, service: BranchServiceDep, cache: CacheDep, _claims: WriteAdminDep):
