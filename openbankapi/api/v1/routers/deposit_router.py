"""POST /admin/deposits — sync admin cash deposit with FX and waiter."""
...
from openbankapi.api.v1.dtos.deposit_dto import DepositRequestDTO, DepositResponseDTO
from openbankapi.config.dependencies import (
    DepositStatusRegistryDep,
...
@router.post(
    "/admin/deposits",
    response_model=DepositResponseDTO,
    response_model_exclude_none=True,
    status_code=200,
    summary="Admin cash deposit",
    description="Credits an account synchronously; FX applied when needed, waiter on deposit-status.",
)
async def create_deposit(
    ...
    registry: DepositStatusRegistryDep,
    ...
    if body.currency == account.currency:
        ...
        quote = convert(body.amount, body.currency, account.currency, "credit", rates)
        amount_applied = quote["final_amount"]
        applied_rate = quote["applied_rate"]

    request_id = str(uuid.uuid4())
    # Register waiter before publish to avoid race
    wait_task = asyncio.create_task(registry.wait_for(request_id, timeout=10.0))
    ...
    if result is None:
        raise HTTPException(status_code=504, detail="deposit confirmation timeout")

    # result contains new_balance and maybe amount_applied/applied_rate echo
